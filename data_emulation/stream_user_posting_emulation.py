...
def run_infinite_post_data_loop():
    while True:
        ...
        with engine.connect() as connection:
            ...
            pin_invoke_url = "https://qklaofl8zd.execute-api.us-east-1.amazonaws.com/Development/streams/streaming-0a40ea42f8d1-pin/record"

            for row in pin_selected_row:
                pin_df = dict(row._mapping)
                pin_payload = json.dumps({
                    "StreamName": "streaming-0a40ea42f8d1-pin",
                    "Data": {
                        "index": pin_df["index"], "unique_id": pin_df["unique_id"], "title": pin_df["title"],
            ...
            geo_invoke_url = "https://qklaofl8zd.execute-api.us-east-1.amazonaws.com/Development/streams/streaming-0a40ea42f8d1-geo/record"

            for row in geo_selected_row:
                geo_df = dict(row._mapping)
                geo_payload = json.dumps({
                    "StreamName": "streaming-0a40ea42f8d1-geo",
                    "Data": {
                        "ind": geo_df["ind"], "timestamp": (geo_df["timestamp"]).isoformat(), "latitude": geo_df["latitude"],
                        "longitude": geo_df["longitude"], "country": geo_df["country"]
                        },
                    "PartitionKey": "test_geo_stream"
                    })
            ...
            user_invoke_url = "https://qklaofl8zd.execute-api.us-east-1.amazonaws.com/Development/streams/streaming-0a40ea42f8d1-user/record"

            for row in user_selected_row:
                user_df = dict(row._mapping)
                user_payload = json.dumps({
                    "StreamName": "streaming-0a40ea42f8d1-user",
                    "Data": {
                        "ind": user_df["ind"], "first_name": user_df["first_name"], "last_name": user_df["last_name"], 
                        "age": user_df["age"], "date_joined": (user_df["date_joined"]).isoformat()
                        },
                    "PartitionKey": "test_user_stream"
                    })
